...
* EDL-10: Course-scoped role management (``GET/POST /admin-portal/api/v1/roles/``).
...
* EDL-8 / EDL-9: Enroll and unenroll endpoints
  (``POST /admin-portal/api/v1/enrollments/enroll/`` and ``.../unenroll/``).
...
* EDL-7: Deactivate/reactivate endpoints
  (``POST /admin-portal/api/v1/users/<username>/deactivate|reactivate/``). Sets
...
* EDL-6: User directory (``GET /admin-portal/api/v1/users/``). Paginated
...
* EDL-5: Create-user endpoint (``POST /admin-portal/api/v1/users/``). Reuses the
  ...
  follows ``ADMIN_PORTAL_PASSWORD_MODE``: ``link`` (default) creates an
...
* EDL-2: EDL-admin access gate. Adds the ``edl_admin`` Django group (via data
  migration), an ``IsEdlAdmin`` DRF permission + ``AdminPortalAPIView`` base, and
  ...
  default (``ADMIN_PORTAL_SUPERUSER_BYPASS``). Adds a gated ``api/v1/me/`` endpoint.

0.1.0
-----

* EDL-1: Initial ``lms.djangoapp`` plugin scaffold — app registration, plugin
  settings (common/production/test), URL wiring at ``^admin-portal/`` with a
  browser landing page at ``/admin-portal/`` and a public REST health endpoint at
  ``/admin-portal/api/v1/health/``, with tests.
