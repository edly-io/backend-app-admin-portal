"""Audit helper for the admin_portal plugin.

Call :func:`record_action` from every mutating panel operation. It writes an
:class:`~admin_portal.models.EdlAdminAuditLog` row and emits a matching
...
def _emit_event(entry):
    ...
    try:  # pragma: no cover - eventtracking is platform-provided
        ...
        return
    try:
        tracker.emit(f'admin_portal.{entry.action}', {
        ...
        log.exception('admin_portal: failed to emit tracking event for audit %s', entry.pk)
