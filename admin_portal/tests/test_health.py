...
class HealthViewTests(APITestCase):
    ...
    def test_health_returns_ok(self):
        response = self.client.get(reverse('admin_portal:v1:health'))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['status'], 'ok')
        self.assertEqual(response.data['service'], 'admin-portal')
        self.assertEqual(response.data['version'], __version__)

    def test_health_url_under_api_v1(self):
        self.assertEqual(reverse('admin_portal:v1:health'), '/admin-portal/api/v1/health/')
