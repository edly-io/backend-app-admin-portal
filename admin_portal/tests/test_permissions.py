...
class GateMixin:
    ...
    def make_user(self, username, admin=False, superuser=False):
        ...
        if superuser:
            ...
            user.save()
        if admin:
            user.groups.add(Group.objects.get(name=settings.ADMIN_PORTAL_ADMIN_GROUP))
        return user
...
class AdminGroupMigrationTests(APITestCase):
    ...
    def test_group_exists(self):
        self.assertTrue(Group.objects.filter(name=settings.ADMIN_PORTAL_ADMIN_GROUP).exists())
...
class MeEndpointGateTests(GateMixin, APITestCase):
    ...
    def setUp(self):
        self.url = reverse('admin_portal:v1:me')
...
class LandingPageGateTests(GateMixin, APITestCase):
    ...
    def setUp(self):
        self.url = reverse('admin_portal:index')
