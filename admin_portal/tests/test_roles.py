...
class RolesGateTests(APITestCase):
    def test_anonymous_get_is_forbidden(self):
        self.assertEqual(self.client.get(reverse('admin_portal:v1:roles')).status_code,
                         status.HTTP_403_FORBIDDEN)

    def test_anonymous_post_is_forbidden(self):
        self.assertEqual(self.client.post(reverse('admin_portal:v1:roles'), {}, format='json').status_code,
                         status.HTTP_403_FORBIDDEN)
...
class RoleCatalogTests(APITestCase):
    ...
    def test_catalog_lists_only_course_roles(self):
        response = self.client.get(reverse('admin_portal:v1:roles'))
...
class RoleActionTests(APITestCase):
    ...
    def setUp(self):
        self.url = reverse('admin_portal:v1:roles')
