...
class StandingGateTests(APITestCase):
    def test_anonymous_is_forbidden(self):
        url = reverse('admin_portal:v1:user-deactivate', args=['someone'])
...
class StandingTests(APITestCase):
    ...
    def test_deactivate_calls_seam_and_audits(self):
        url = reverse('admin_portal:v1:user-deactivate', args=['learner'])
    ...
    def test_reactivate_calls_seam_and_audits(self):
        url = reverse('admin_portal:v1:user-reactivate', args=['learner'])
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_unknown_user_is_404(self):
        url = reverse('admin_portal:v1:user-deactivate', args=['ghost'])
        response = self.client.post(url)
