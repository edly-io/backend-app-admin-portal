"""Access control for the edl_panel plugin (EDL-2).
...
bypass the check (``EDL_PANEL_SUPERUSER_BYPASS``).
"""
...
def user_is_edl_admin(user):
    ...
    if not (user and user.is_authenticated):
        return False
    if getattr(settings, 'EDL_PANEL_SUPERUSER_BYPASS', True) and user.is_superuser:
        return True
    group_name = getattr(settings, 'EDL_PANEL_ADMIN_GROUP', 'edl_admin')
