...
The platform seam (``edl_panel.edxapp``) is patched so the suite runs without
...
class CreateUserGateTests(APITestCase):
    def test_anonymous_is_forbidden(self):
        response = self.client.post(reverse('edl_panel:v1:users'), {}, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)


class CreateUserTests(APITestCase):
    def setUp(self):
        self.url = reverse('edl_panel:v1:users')
    ...
    # --- copy mode ---
    @override_settings(EDL_PANEL_PASSWORD_MODE='copy')
